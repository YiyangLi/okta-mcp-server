...
function extractPrimitives(obj) {
    ...
    if (Array.isArray(obj)) {
        return obj.map(item => typeof item !== 'object' || item === null ? item : extractPrimitives(item)).filter(item => item !== undefined);
    }
    // Handle objects
    const result = {};
...
// === USER TOOLS ===
// List Users
server.tool("okta_list_users_make_request", {
...
// Create User
server.tool("okta_create_user_make_request", {
    profile: z.object({
        firstName: z.string(),
        lastName: z.string(),
...
// Get User
server.tool("okta_get_user_make_request", {
...
// Update User
server.tool("okta_update_user_make_request", {
    userId: z.string(),
    profile: z.object({
...
// Delete User
server.tool("okta_delete_user_make_request", {
    userId: z.string()
}, async ({ userId }) => {
    try {
        // First deactivate
...
// === GROUP TOOLS ===
// List Groups
server.tool("okta_list_groups_make_request", {
    limit: z.number().optional().describe("Number of results to return (default 20)"),
...
// Create Group
server.tool("okta_create_group_make_request", {
    profile: z.object({
...
server.tool("okta_assign_user_to_group_make_request", {
    groupId: z.string(),
    userId: z.string()
...
// === APPLICATION TOOLS ===
// List Applications
server.tool("okta_list_applications_make_request", {
    limit: z.number().optional().describe("Number of results to return (default 20)"),
...
server.tool("okta_assign_user_to_application_make_request", {
    appId: z.string(),
    userId: z.string(),
...
server.tool("okta_assign_group_to_application_make_request", {
    appId: z.string(),
    groupId: z.string()
}, async ({ appId, groupId }) => {
    try {
...
// Delete Application
server.tool("okta_delete_application_make_request", {
...
server.tool("okta_deactivate_application_make_request", {
